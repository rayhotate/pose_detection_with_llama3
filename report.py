...
def generate_report():
    ...
    with open('README.md', 'w') as f:
        ...
        f.write("```mermaid\n")
        f.write("graph TD\n")
        f.write("    A[Input Image] --> B[Load Image]  %% Load the image from the specified path\n")
        f.write("    B --> C[Describe Image]  %% Generate an initial description of the image\n")
        f.write("    C --> D[Generate Initial Analysis]  %% Create a preliminary analysis based on the description\n")
        f.write("    D --> E[Detailed Prompt Analysis]  %% Apply a detailed prompt for further analysis\n")
        f.write("    E --> F[Generate Full Response]  %% Generate a comprehensive response\n")
        f.write("    F --> G[Determine Turning Assistance]  %% Evaluate if turning assistance is occurring\n")
        f.write("    G --> H[Binary Classification]  %% Classify the scenario as True or False\n")
        f.write("    H --> I[Reasoning Output]  %% Output the reasoning behind the classification\n")
        f.write("```\n\n")
